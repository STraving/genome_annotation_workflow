...
### 1. Quality check using Fastp

For more information on Fastp see https://github.com/OpenGene/fastp
...
We run the assembly with the `--isolate` flag.
...
If your assembly contains alot of smaller contigs ≥ 1000 bp it is probably helpful to filter these out. You can remove them with seqtk.
  
        seqtk seq -L 1000 contigs.fasta > contigs_1kb.fasta
...
Example for a single genome, therefore the `--skip_ani_screen` flag:
   
      gtdbtk classify_wf --genome_dir genomes/ --out_dir GTDB-Tk --cpus 2 --skip_ani_screen --extension fasta --scratch_dir gtdbtk_scratch

Example with multiple genomes classified in batch, don't skip the ani screen, use `--mash_db` flag. the db lives with the GTDBTK dir of the installation:

      gtdbtk classify_wf --genome_dir genomes/ --out_dir GTDB-Tk --cpus 6 --extension fasta --scratch_dir gtdbtk_scratch --mash_db path/to/gtdb_ref_sketch.msh
